    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!("  translate --dir <path>     Translate all Legado JSON files in a directory");
                return;
            }
            _ => {}
        }
    ...
    for source in &sources {
        ...
        match nexus_engine::translator::rules::generate_source(source) {
            Ok(js) => {
                ...
                let output_path = path.parent().unwrap_or(&PathBuf::from("."))
                    .join("generated")
                    .join(format!("{}.js", id));
                ...
                println!("Syntax check: {}", if check_js_syntax(&js) { "✅" } else { "⚠️  Manual check needed" });
                println!();
            }
            Err(e) => {
