...
impl NodeWorker {
    fn spawn(config: &NodePoolConfig) -> Result<Self, EngineError> {
        ...
        Ok(Self { process, stdin, next_id: AtomicUsize::new(1) })
    }
    ...
    fn send_request(&mut self, method: &str, params: &str) -> Result<u64, EngineError> {
        ...
        writeln!(self.stdin, "{}", request).map_err(|e| {
            EngineError::ContentExtractionFailed {
    ...
    fn read_response(&mut self, expected_id: u64, timeout_secs: u64) -> Result<String, EngineError> {
...
impl NodeRunner {
    pub fn new(config: NodePoolConfig) -> Result<Self, EngineError> {
        ...
        Ok(Self { config, workers: Arc::new(Mutex::new(workers)) })
    }

    async fn execute(&self, source_id: &str, method: &str, args: &[(&str, &str)]) -> Result<String, EngineError> {
        let mut workers = self.workers.lock().await;
        let mut worker = workers.pop_front().ok_or_else(|| {
            EngineError::ContentExtractionFailed {
...
impl Runner for NodeRunner {
    async fn search(&self, source_id: &str, keyword: &str, page: u32) -> Result<RunResult<Vec<BookItem>>, EngineError> {
        let response = self.execute(
            source_id, "search",
            &[("keyword", keyword), ("page", &page.to_string())],
        ).await?;

        let items: Vec<BookItem> = serde_json::from_str(&extract_result(&response))
            .map_err(|e| EngineError::ContentExtractionFailed {
                reason: format!("parse search: {}", e),
            })?;

        Ok(RunResult { data: items, needs_browser: None })
    }

    async fn book_info(&self, source_id: &str, book_url: &str) -> Result<RunResult<BookInfo>, EngineError> {
        let response = self.execute(source_id, "bookInfo", &[("bookUrl", book_url)]).await?;
        let info: BookInfo = serde_json::from_str(&extract_result(&response))
            .map_err(|e| EngineError::ContentExtractionFailed {
                reason: format!("parse bookInfo: {}", e),
            })?;
        Ok(RunResult { data: info, needs_browser: None })
    }

    async fn chapters(&self, source_id: &str, toc_url: &str) -> Result<RunResult<Vec<Chapter>>, EngineError> {
        let response = self.execute(source_id, "chapterList", &[("tocUrl", toc_url)]).await?;
        let chapters: Vec<Chapter> = serde_json::from_str(&extract_result(&response))
            .map_err(|e| EngineError::ContentExtractionFailed {
                reason: format!("parse chapters: {}", e),
            })?;
        Ok(RunResult { data: chapters, needs_browser: None })
    }

    async fn content(&self, source_id: &str, chapter_url: &str) -> Result<RunResult<String>, EngineError> {
        let response = self.execute(source_id, "chapterContent", &[("chapterUrl", chapter_url)]).await?;
        let content: String = serde_json::from_str(&extract_result(&response))
            .map_err(|e| EngineError::ContentExtractionFailed {
                reason: format!("parse content: {}", e),
            })?;
        Ok(RunResult { data: content, needs_browser: None })
    }
}
