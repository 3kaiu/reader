 ...
 *
 * 推荐标准：
 * - 参数量：1B / 1.5B / 2B / 3B / 7B / 8B
 * - 量化：Q4
 * - VRAM：< 7GB（允许 3B 与 7B/8B Q4 端侧运行）
 * - 主流系列：Qwen 2.5、Llama 3.2（中文友好）
 * - 优先 3B（轻量），可附加 7B/8B（重型）
 *
 * @returns 推荐的模型 ID 数组
 * @example
 * ```typescript
 * const recommended = getRecommendedModels()
 * // ['Qwen2.5-3B-Instruct-q4f16_1-MLC', 'Qwen2.5-7B-Instruct-q4f16_1-MLC']
 * ```
 */
      ...
      .filter(
        (
          ...
          const isChineseUnfriendly =
            ...
            (id.includes("llama") &&
              !id.includes("llama-3.2") &&
              !id.includes("llama-3.1")); // 只保留 Llama 3.1+ 和 3.2（较新版本对中文支持更好）
...
export const useAIStore = defineStore("ai", () => {
  ...
  async function chat(
    ...
    // 构建请求参数
    // 获取上下文窗口大小（如果 settings store 可用）
    let contextWindow = AI_DEFAULT_CONTEXT_WINDOW;
    try {
      const { useSettingsStore } = await import("../stores/settings");
      const settingsStore = useSettingsStore();
      contextWindow =
        settingsStore.config.aiParams.contextWindow ||
        AI_DEFAULT_CONTEXT_WINDOW;
    } catch {
      // 使用默认值
    }
    ...
    try {
      ...
      if (onStream) {
        // 流式输出
        let fullResponse = "";
        // WebLLM 的类型定义可能与实际使用不完全匹配，使用类型断言
        const asyncChunkGenerator = await (
          engine.value.chat.completions.create as any
        )({
          ...requestParams,
          stream: true,
        ...
        performance.value = {
          tokensPerSecond:
            tokenCount > 0 ? Math.round(tokenCount / elapsed) : 0,
...
- 帮助用户快速回忆`;
  ...
  // 谐音识别与映射分析
  async function detectHomophones(content: string): Promise<HomophoneItem[]> {
...
如果没有发现任何映射，返回空数组 []。`;
    ...
    try {
      ...
      try {
        ...
        parsed = JSON.parse(cleanJson) as
          | HomophoneItem[]
          | { results?: HomophoneItem[] };
      } catch {
        // 如果直接解析失败，尝试提取数组
      ...
      return rawResults
        ...
        .filter(
          (item) =>
            item.original &&
            item.guess.length > 0 &&
            !item.guess.includes(item.original) // 过滤掉无效项（guess 中包含 original）
        );
    } catch (e) {
      logger.error("谐音识别失败", e as Error, {
        function: "detectHomophones",
      });
      return [];
    }
  }
