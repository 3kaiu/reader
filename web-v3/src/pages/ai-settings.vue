...
import { useAI, RECOMMENDED_MODELS } from '@/composables/useAI'
...
<template>
  <div class="min-h-screen bg-background">
    ...
    <main class="container mx-auto max-w-screen-md px-4 py-8">
      <!-- WebGPU 不支持提示 -->
      <div v-if="!ai.isSupported.value" class="p-6 rounded-2xl bg-destructive/10 text-destructive mb-8">
      ...
      <!-- 当前模型 -->
      <section v-if="ai.isModelLoaded.value" class="mb-8">
        ...
        <div class="p-5 rounded-2xl border bg-card flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="font-medium">{{ ai.currentModel.value?.split('-').slice(0, 2).join(' ') }}</h3>
              <p class="text-sm text-muted-foreground">已加载，可在阅读时使用 AI 功能</p>
            </div>
          </div>
          <Button variant="outline" size="sm" @click="ai.unloadModel()">
      ...
      <!-- 加载进度 -->
      <section v-if="ai.isLoading.value" class="mb-8">
        <h2 class="text-sm font-medium text-muted-foreground mb-4">正在下载</h2>
        <div class="p-5 rounded-2xl border bg-card">
          <div class="flex items-center gap-4 mb-4">
            <Loader2 class="w-8 h-8 animate-spin text-primary" />
            <div class="flex-1">
              <p class="font-medium mb-1">{{ ai.loadStatus.value }}</p>
              <div class="w-full bg-muted rounded-full h-2">
                <div 
                  class="bg-primary h-2 rounded-full transition-all"
                  :style="{ width: `${ai.loadProgress.value}%` }"
                />
              </div>
            </div>
            <span class="text-sm text-muted-foreground font-medium">{{ ai.loadProgress.value }}%</span>
          </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div>
                  <h3 class="font-medium flex items-center gap-2">
                    {{ model.name }}
                    <span 
                      v-if="ai.currentModel.value === model.id"
