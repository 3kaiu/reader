...
use nexus_core::{AiMappingRule, AiAnalysisHistory};
...
pub async fn list_mapping_rules(
    ...
    match state.store.get_ai_mapping_rules() {
...
pub async fn save_mapping_rule(
    ...
    match state.store.save_ai_mapping_rule(&rule) {
...
pub async fn delete_mapping_rule(
    ...
    match state.store.delete_ai_mapping_rule(&id) {
        Ok(_) => Json(ApiResponse::success(())),
        Err(e) => Json(ApiResponse::error(&e.to_string())),
    }
}
...
pub async fn list_analysis_history(
    ...
    match state.store.get_ai_analysis_history(limit) {
...
pub async fn save_analysis_history(
    ...
    match state.store.save_ai_analysis_history(&history) {
        Ok(_) => Json(ApiResponse::success(())),
        Err(e) => Json(ApiResponse::error(&e.to_string())),
    }
}
...
pub async fn clear_analysis_history(
    State(state): State<AppState>,
) -> Json<ApiResponse<()>> {
    match state.store.clear_ai_analysis_history() {
        Ok(_) => Json(ApiResponse::success(())),
        Err(e) => Json(ApiResponse::error(&e.to_string())),
    }
}
