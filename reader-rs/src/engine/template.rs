...
use super::preprocessor::{TemplateExpr, NativeApi, PreprocessedUrl};
...
impl TemplateExecutor {
    ...
    pub fn execute_expr(&self, expr: &TemplateExpr, ctx: &TemplateContext) -> Result<String> {
        match expr {
            ...
            TemplateExpr::Variable(name) => {
                Ok(ctx.get(name).cloned().unwrap_or_default())
            }
    ...
    fn execute_js_fallback(&self, method: &str, args: &[String], ctx: &TemplateContext) -> Result<String> {
        if let Some(ref js) = self.js_executor {
            // Build JS call
    ...
    fn eval_simple_math(&self, expr: &str) -> Result<i64> {
        ...
        // Just a number
        expr.parse().map_err(|e| anyhow::anyhow!("Parse error: {}", e))
    }
}
...
mod tests {
    ...
    fn create_executor() -> TemplateExecutor {
        ...
        let native_api = Arc::new(NativeApiProvider::new(cm));
    ...
    fn test_literal() {
        ...
        let result = exec.execute_expr(&TemplateExpr::Literal("hello".to_string()), &ctx).unwrap();
    ...
    fn test_variable() {
        let exec = create_executor();
        let ctx = TemplateContext::new()
            .with_var("key", "test_value");
        
        let result = exec.execute_expr(&TemplateExpr::Variable("key".to_string()), &ctx).unwrap();
    ...
    fn test_page_calculation() {
        ...
        let result = exec.execute_expr(
            &TemplateExpr::JsExpr("(page-1)*20".to_string()),
            &ctx
        ).unwrap();
