...
impl JsExecutor {
    ...
    fn register_utils(&self, ctx: &Ctx) -> rquickjs::Result<()> {
        ...
        aes_obj.set(
            ...
            Function::new(
                ctx.clone(),
                move |data: String, key: String, iv: String| -> String {
                    api.execute(
                        &NativeApi::AesEncode {
        ...
        java_obj.set(
            "put",
            Function::new(ctx.clone(), move |key: String, value: Value| {
                let ctx = value.ctx();
        ...
        java_obj.set(
            "t2s",
            Function::new(ctx.clone(), |text: String| -> String { text })?,
        )?;
        java_obj.set(
            "s2t",
            Function::new(ctx.clone(), |text: String| -> String { text })?,
        ...
        java_obj.set(
            "base64Encode",
            Function::new(ctx.clone(), move |val: Value| -> String {
                let s = value_to_string_js(&val);
                api.execute(&NativeApi::Base64Encode, &[s])
                    .unwrap_or_default()
            })?,
        )?;
        ...
        java_obj.set(
            ...
            Function::new(ctx.clone(), move |val: Value| -> String {
                let s = value_to_string_js(&val);
                api.execute(&NativeApi::Base64Decode, &[s])
                    .unwrap_or_default()
            })?,
        )?;
        ...
        java_obj.set(
            ...
            Function::new(ctx.clone(), |str: String| -> String {
                use base64::Engine;
                base64::engine::general_purpose::STANDARD
                    .decode(str.as_bytes())
        ...
        java_obj.set(
            "base64EncodeWithFlags",
            Function::new(ctx.clone(), |str: String, flags: i32| -> String {
                use base64::Engine;
                if flags == 2 {
        ...
        java_obj.set(
            "base64DecodeToByteArrayWithFlags",
            Function::new(ctx.clone(), |str: String, flags: i32| -> String {
                use base64::Engine;
                let engine = if flags & 8 != 0 {
                    &base64::engine::general_purpose::URL_SAFE
        ...
        java_obj.set(
            ...
            Function::new(ctx.clone(), move |val: Value| -> String {
                let s = value_to_string_js(&val);
                api.execute(&NativeApi::Md5Encode16, &[s])
                    .unwrap_or_default()
            })?,
        )?;
        ...
        java_obj.set(
            "decodeURI",
            Function::new(ctx.clone(), |val: Value| -> String {
                let s = value_to_string_js(&val);
        ...
        java_obj.set(
            "encodeURIWithEnc",
            Function::new(ctx.clone(), |str: String, enc: String| -> String {
        ...
        java_obj.set(
            "utf8ToGbk",
            Function::new(ctx.clone(), move |str: String| -> String {
                api.execute(&NativeApi::Utf8ToGbk, &[str])
                    .unwrap_or_default()
            })?,
        )?;
        ...
        java_obj.set(
            "androidId",
            Function::new(ctx.clone(), || -> String { String::new() })?,
        )?;

        // java.logType(any) - Log the type of value
        java_obj.set(
            "logType",
            Function::new(ctx.clone(), |val: Value| {
                let type_name = if val.is_undefined() {
                    "undefined"
                } else if val.is_null() {
                    "null"
                } else if val.is_bool() {
        ...
        java_obj.set(
            ...
            Function::new(
                ctx.clone(),
                |html: Option<String>, url: Option<String>, js: Option<String>| -> String {
                    use super::webview::WebViewExecutor;

                    // Try to create WebView executor
                    match WebViewExecutor::new() {
                        Ok(executor) => executor
                            .render(html.as_deref(), url.as_deref(), js.as_deref())
                            .unwrap_or_else(|e| {
        ...
        java_obj.set(
            "ajaxAll",
            Function::new(ctx.clone(), |urls: Value| -> Vec<String> {
                let _ctx = urls.ctx();
                ...
                let url_list: Vec<String> = if urls.is_array() {
                    ...
                    let len = arr.len();
                    (0..len).filter_map(|i| arr.get::<String>(i).ok()).collect()
        ...
        java_obj.set(
            "cacheFile",
            Function::new(ctx.clone(), move |url: String, save_time: i32| -> String {
                use std::fs;
        ...
        java_obj.set(
            "readTxtFileWithCharset",
            Function::new(ctx.clone(), |path: String, charset: String| -> String {
        ...
        java_obj.set(
            "getZipStringContent",
            Function::new(ctx.clone(), |url: String, path: String| -> String {
                use std::io::Read;
                use zip::ZipArchive;
        ...
        java_obj.set(
            "getZipStringContentWithCharset",
            Function::new(
                ctx.clone(),
                |url: String, path: String, charset: String| -> String {
                    use encoding_rs::{GB18030, GBK, UTF_8};
                    use std::io::Read;
                    use zip::ZipArchive;
        ...
        java_obj.set(
            "unzipFile",
            Function::new(ctx.clone(), move |zip_path: String| -> String {
                use std::fs;
                use zip::ZipArchive;
        ...
        java_obj.set(
            ...
            Function::new(
                ctx.clone(),
                |method: String, url: String, body: String, headers: String| -> String {
        ...
        java_obj.set(
            "timeFormat",
            Function::new(ctx.clone(), move |time: i64| -> String {
                api.execute(&NativeApi::TimeFormat(None), &[time.to_string()])
                    .unwrap_or_default()
            })?,
        )?;
        ...
        java_obj.set(
            ...
            Function::new(
                ctx.clone(),
                |data: String, key: String, _transformation: String, iv: String| -> String {
        ...
        java_obj.set(
            ...
            Function::new(
                ctx.clone(),
                move |data: String, key: String, _transformation: String, iv: String| -> String {
                    api.execute(
                        &NativeApi::AesDecode {
                            transformation: String::new(),
                            iv,
        ...
        let font_cache = std::sync::Arc::new(std::sync::Mutex::new(HashMap::<
            String,
            super::query_ttf::QueryTTF,
        >::new()));
        let font_cache_for_query = font_cache.clone();

        java_obj.set(
            "queryTTF",
            Function::new(ctx.clone(), move |font_data: String| -> String {
                ...
                let bytes: Vec<u8> =
                    if font_data.starts_with("http://") || font_data.starts_with("https://") {
                        // Download from URL
                        std::thread::spawn(move || {
                            let client = reqwest::blocking::Client::builder()
                                .timeout(std::time::Duration::from_secs(30))
        ...
        java_obj.set(
            ...
            Function::new(
                ctx.clone(),
                move |text: String, font_id1: String, font_id2: String| -> String {
        ...
        java_obj.set(
            "queryBase64TTF",
            Function::new(ctx.clone(), move |base64_data: String| -> String {
                use super::query_ttf::QueryTTF;
                use base64::Engine;
        ...
        java_obj.set(
            "digestHex",
            Function::new(ctx.clone(), |data: String, algorithm: String| -> String {
                use sha1::Sha1;
        ...
        java_obj.set(
            ...
            Function::new(
                ctx.clone(),
                move |time: i64, format: String, sh: i32| -> String {
        ...
        java_obj.set(
            "downloadFile",
            Function::new(ctx.clone(), move |content: String, url: String| -> String {
                use std::fs;
        ...
        java_obj.set(
            "getCookie",
            Function::new(ctx.clone(), move |tag: String, key: String| -> String {
                let cookie_key = format!("cookie_{}", tag);
                let full_cookie = cookie_cache_get
                    .lock()
                    .ok()
...
mod tests {
    ...
    fn test_java_cookies() {
        ...
        // Mock cookies in cache
        {
            ...
            cache.insert(
                "cookie_http://example.com".to_string(),
        ...
        let full = executor
            .eval("java.getCookie('http://example.com', '')")
            .unwrap();
