...
impl Parser for JsoupDefaultParser {
    fn get_string(&self, content: &str, rule: &str) -> Result<String> {
        ...
        tracing::debug!(
            "JsoupDefaultParser.get_string: rule='{}', selector='{}', attr='{}', content_len={}",
            rule,
            selector_str,
            attr,
            content.len()
        ...
        if !rule.contains('@') {
            ...
            if attr_names.iter().any(|&a| a == rule) {
                ...
                if let Some(first_element) = root
                    .descendants()
        ...
        if let Ok(selector) = Selector::parse(&selector_str) {
            ...
            tracing::debug!(
                "JsoupDefaultParser: CSS selector '{}' matched {} elements",
                selector_str,
                matches.len()
            );
            if !matches.is_empty() {
                if attr == "text" || attr == "" {
                    ...
                    let texts: Vec<String> = matches
                        ...
                        .collect();
                    tracing::debug!(
                        "JsoupDefaultParser: Joining {} non-empty texts",
                        texts.len()
                    );
    ...
    fn get_elements(&self, content: &str, rule: &str) -> Result<Vec<String>> {
        ...
        tracing::debug!(
            "get_elements: rule='{}', css_selector='{}'",
            rule,
            css_selector
        );
        ...
        let results: Vec<String> = matches.iter().map(|el| el.html()).collect();
    ...
    for node in element.children() {
        if let Some(el) = ElementRef::wrap(node) {
            ...
            if tag == "br" {
                ...
                "p", "div", "li", "dd", "dt", "h1", "h2", "h3", "h4", "h5", "h6",
            ]
            .contains(&tag)
            {
...
fn parse_jsoup_rule(rule: &str) -> Result<(Vec<SelectorSegment>, String)> {
    ...
    let segments: Vec<SelectorSegment> = selector
        .split_whitespace()
...
fn apply_selectors<'a>(
    ...
    for segment in segments {
        ...
        for parent in &current {
            let tag_selector = if segment.tag == "*" {
                "*".to_string()
            } else {
                segment.tag.clone()
            };
                    ...
                    match modifier {
                        ...
                        SelectorModifier::Exclude(idx) => {
                            if let Some(el_to_remove) = apply_index(&candidates, *idx) {
                                if let Some(pos) =
                                    candidates.iter().position(|x| *x == el_to_remove)
                                {
...
fn parse_modifier(piece: &str, modifiers: &mut Vec<SelectorModifier>) {
    ...
    // If it parses as a number, it could be EITHER a class name (like "-1") OR an index
    // Use Ambiguous to let runtime decide based on whether class matches exist
...
mod tests {
    ...
    fn test_jsoup_id_selector() {
        ...
        assert!(
            elements[0].contains("<a"),
            "First element should be <a>, got: {}",
            elements[0]
        );
        assert!(
            elements[0].contains("href="),
            "First element should have href, got: {}",
            elements[0]
        );
    ...
    fn test_jsoup_dd_selector() {
        ...
        assert_eq!(
            elements.len(),
            2,
            "Should find 2 <a> elements from dl/dd structure"
        );
