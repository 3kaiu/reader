...
static JAVA_CALL_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^java\.(\w+)\s*\((.*)?\)$").unwrap());
...
pub enum NativeApi {
    ...
    // ============== Crypto - 3DES ==============
    TripleDesDecodeStr,
    TripleDesDecodeArgsBase64,
    TripleDesEncodeBase64,
    TripleDesEncodeArgsBase64,
...
impl SourcePreprocessor {
    ...
    pub fn new() -> Self {
        ...
        // Cookie
        native_apis.insert("getCookie".to_string(), |_| NativeApi::GetCookie);
        native_apis.insert("setCookie".to_string(), |_| NativeApi::SetCookie);

        // Encoding/Encryption
        native_apis.insert("hexEncode".to_string(), |_| NativeApi::HexEncode);
        native_apis.insert("hexDecode".to_string(), |_| NativeApi::HexDecode);
        native_apis.insert("aesEncode".to_string(), |_| NativeApi::AesEncode);
        native_apis.insert("aesDecode".to_string(), |_| NativeApi::AesDecode);
        native_apis.insert("desEncode".to_string(), |_| NativeApi::DesEncode);
        native_apis.insert("desDecode".to_string(), |_| NativeApi::DesDecode);

        // HTTP
        native_apis.insert("httpGet".to_string(), |_| NativeApi::HttpGet);
        native_apis.insert("httpPost".to_string(), |_| NativeApi::HttpPost);
        native_apis.insert("ajax".to_string(), |_| NativeApi::HttpGet);
        native_apis.insert("connect".to_string(), |_| NativeApi::HttpGet);

        // Storage
        native_apis.insert("putVariable".to_string(), |_| NativeApi::SourceVarSet);
        native_apis.insert("getVariable".to_string(), |_| NativeApi::SourceVarGet);
        native_apis.insert("cachePut".to_string(), |_| NativeApi::CacheSet);
        native_apis.insert("cacheGet".to_string(), |_| NativeApi::CacheGet);

        // Misc
        native_apis.insert("log".to_string(), |_| NativeApi::Log);
    ...
    pub fn parse_template(&self, text: &str) -> Vec<TemplateExpr> {
        ...
        for cap in TEMPLATE_REGEX.captures_iter(text) {
            ...
            if full_match.start() > last_end {
                parts.push(TemplateExpr::Literal(
                    text[last_end..full_match.start()].to_string(),
                ));
            }
    ...
    fn parse_template_expr(&self, expr: &str) -> TemplateExpr {
        ...
        if let Some(caps) = JAVA_CALL_REGEX.captures(expr) {
            ...
            if let Some(api_factory) = self.native_apis.get(method_name) {
                ...
                return TemplateExpr::NativeCall { api, args };
            }

            // Unknown java.xxx() call - check common ones
            // (Handled via generic register now)
            ...
            .map(|arg| {
                ...
                Box::new(if arg.starts_with('"') || arg.starts_with('\'') {
                    TemplateExpr::Literal(
                        self.extract_string_literal(&TemplateExpr::Literal(arg.to_string())),
                    )
    ...
    fn extract_string_literal(&self, expr: &TemplateExpr) -> String {
        match expr {
            TemplateExpr::Literal(s) => s.trim_matches(|c| c == '"' || c == '\'').to_string(),
            _ => String::new(),
        }
    }
    ...
    fn extract_put_vars(&self, rule: &str) -> (String, HashMap<String, String>) {
        ...
        for cap in put_regex.captures_iter(rule) {
            ...
            // Try to parse as JSON
            if let Ok(json) =
                serde_json::from_str::<serde_json::Value>(&format!("{{{}}}", json_content))
            {
    ...
    fn detect_rule_type(&self, rule: &str) -> (PreprocessedRuleType, String) {
        ...
        // JS Rule
            ...
            (
                PreprocessedRuleType::JavaScript,
                rule[4..rule.len() - 5].to_string(),
            )
        }
        // Regex Rule
        else if rule.starts_with(":") || rule.starts_with("##") {
...
mod tests {
    ...
    fn test_preprocess_simple_url() {
        ...
        assert!(
            matches!(&result.parts[0], TemplateExpr::Literal(s) if s == "https://example.com/search?q=")
        );
    ...
    fn test_get_cookie_detection() {
        let pp = SourcePreprocessor::new();
        let result = pp.preprocess_url(
            r#"https://example.com?cookie={{java.getCookie("http://site.com", "userid")}}"#,
        );

        assert_eq!(result.parts.len(), 2);
        if let TemplateExpr::NativeCall { api, args } = &result.parts[1] {
            assert_eq!(api, &NativeApi::GetCookie);
            assert_eq!(args.len(), 2);
            // Verify first arg is literal
            if let TemplateExpr::Literal(url) = &*args[0] {
                assert_eq!(url, "http://site.com");
            } else {
                panic!("Expected literal url arg");
            }
        } else {
