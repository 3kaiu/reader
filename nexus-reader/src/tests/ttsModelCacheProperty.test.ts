...
vi.mock('@/utils/adaptiveAssetLoader', () => ({
  AdaptiveLoader: {
    loadHeavyModule: (...args: any[]) => mockAdaptiveLoader.loadHeavyModule(...args)
  }
}))
...
describe('TTS Model Cache Properties', () => {
        ...
        createBufferSource: vi.fn().mockImplementation(() => {
          const source = {
            ...
            start: vi.fn().mockImplementation(() => {
              if (source.onended) setTimeout(source.onended, 0)
            }),
            stop: vi.fn(),
            disconnect: vi.fn(),
            onended: null as any
  ...
  describe('Property 16: TTS Model Cache Consistency', () => {
    it('should cache TTS models on first use', async () => {
      ...
      const mockPiperTTS = {
        PiperWebWorkerEngine: vi.fn().mockImplementation(function () { return mockEngine }),
        OnnxWebGPUWorkerRuntime: vi.fn(),
        HuggingFaceVoiceProvider: vi.fn()
      }

      mockAdaptiveLoader.loadHeavyModule.mockResolvedValue(mockPiperTTS)
    ...
    it('should handle multiple voice models independently', async () => {
      ...
      // DEBUG: Check if module loader was called
      expect(mockAdaptiveLoader.loadHeavyModule).toHaveBeenCalled()
