...
const defaultConfig: ReaderConfig = {
  ...
  aiParams: {
    ...
    presencePenalty: 0.0, // 默认话题新鲜度
    frequencyPenalty: 0.0, // 默认频率惩罚度
  },
  zenMode: false,
  ttsEngine: "system",
  ttsRate: 1.0,
...
export const useSettingsStore = defineStore("settings", () => {
  ...
  // 数据迁移：确保旧数据有新字段
  if (!config.value.aiParams) {
    config.value = { ...config.value, aiParams: defaultConfig.aiParams };
  }
  ...
  const fontFamilyMap: Record<FontFamily, string> = {
    ...
    heiti: '"PingFang SC", "Microsoft YaHei", "Source Han Sans SC", "Noto Sans CJK SC", sans-serif',
    kaiti: '"LXGW WenKai Screen", KaiTi, STKaiti, "AR PL UKai CN", serif',
    songti: '"Source Han Serif SC", "Noto Serif CJK SC", SimSun, STSong, "AR PL UMing CN", serif',
    fangsong: 'FangSong, STFangsong, serif',
    lxgw: '"LXGW WenKai Screen", KaiTi, STKaiti, serif',
  };

  const currentFontFamily = computed(
    () => fontFamilyMap[config.value.fontFamily] || fontFamilyMap.system
  );

  // 方法
  function updateConfig<K extends keyof ReaderConfig>(
    key: K,
  ...
  function increaseLineHeight() {
    updateConfig(
      "lineHeight",
      Math.min(3, +(config.value.lineHeight + 0.1).toFixed(1))
    );
  }
  ...
  function decreaseLineHeight() {
    updateConfig(
      "lineHeight",
      Math.max(1.2, +(config.value.lineHeight - 0.1).toFixed(1))
    );
  }
