...
export function getOfflineManagerStatus(
  state: OfflineManagerRuntimeState,
): OfflineStatus {
  return state.statusTracker.getStatus(
    state.operationQueue.length,
    state.cacheRegistry.size(),
  )
}

export function isOfflineManagerOnline(
  state: OfflineManagerRuntimeState,
): boolean {
...
export function notifyOfflineStatusListeners(
  state: OfflineManagerRuntimeState,
): void {
  state.statusTracker.notify(
    state.operationQueue.length,
    state.cacheRegistry.size(),
    error => logger.error('Offline status listener error:', { error }),
  )
}
