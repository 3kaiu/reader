...
import { createReaderScrollLoadStateViewBindings } from './reader-scroll-load-state-view-bindings'
import type { ReaderScrollLoadStateEmits } from './reader-scroll-load-state-emit-types'
...
const { showInitialParsing, showLoadingMore, showFinished, showLoadActions } =
  createReaderScrollLoadStateBindings(props)
const { initialLoadingProps, loadingMoreProps, loadActionsBindings } =
...
useIntersectionObserver(
  ...
  ([{ isIntersecting }]) => {
    ...
    if (isIntersecting && !props.isLoadingMore && !props.loadError && props.hasNextChapter) {
...
<template>
  <div ref="rootRef" class="w-full">
    <ReaderScrollLoadingState v-if="showInitialParsing" v-bind="initialLoadingProps" />

    <ReaderScrollLoadingState v-else-if="showLoadingMore" v-bind="loadingMoreProps" />

    <ReaderScrollFinishedState v-else-if="showFinished" />

    <ReaderScrollLoadActions v-else-if="showLoadActions" v-bind="loadActionsBindings" />
  </div>
</template>
