...
function handleSelectionChange() {
  ...
  if (
    props.containerRef &&
    !props.containerRef.contains(range.commonAncestorContainer)
  ) {
    return;
  }
...
async function markCharacter(
  role: "protagonist" | "supporting" | "others" | null
) {
  ...
  try {
    ...
    toast({
      ...
      description: role
        ? `已将「${selectedText.value}」设为${
            role === "protagonist" ? "主角" : "配角"
          }`
        : `已移除「${selectedText.value}」的人物标注`,
    });
    showMenu.value = false;
  } catch (e) {
    toast({
    ...
    <div class="relative -translate-x-1/2 pointer-events-auto">
      ...
      >
        <Button
          variant="ghost"
          size="sm"
          class="rounded-full h-8 px-3 gap-1.5"
          @click="askAI"
        >
      ...
      >
        <Button
          ...
          size="sm"
          class="rounded-full h-8 px-3 gap-1.5"
          @click="markCharacter('protagonist')"
        >
        ...
        <Button
          ...
          size="sm"
          class="rounded-full h-8 px-3 gap-1.5"
          @click="markCharacter('supporting')"
        >
        ...
        <Button
          variant="ghost"
          size="sm"
          class="rounded-full h-8 w-8 p-0 text-muted-foreground"
          @click="showCharMenu = false"
        >
          <X class="w-3.5 h-3.5" />
        </Button>
      </div>
      ...
      >
        ...
        <div
          class="max-h-[200px] overflow-y-auto text-sm leading-relaxed text-foreground/90"
        >
          <div
        ...
        <div
          v-if="aiResponse && !isAnalyzing"
          class="mt-4 pt-3 border-t border-border flex justify-end"
        >
          <Button
            variant="outline"
            size="xs"
            class="h-7 text-[10px] rounded-full"
            @click="showMenu = false"
          >
            完成
          </Button>
