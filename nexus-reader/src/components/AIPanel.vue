...
// 状态
const activeTab = ref<
  "summary" | "homophone" | "chat" | "recap" | "slang" | "memes" | "graph"
>("summary");
const summaryResult = ref("");
const recapResult = ref("");
const homophoneResult = ref<
  Array<{ original: string; guess: string; confidence: number }>
>([]);
...
async function detectHomophones(forceRefresh = false) {
  ...
  try {
    ...
    logger.error("谐音检测失败", e as Error, {
      function: "AIPanel.detectHomophones",
    });
  } finally {
    isProcessing.value = false;
  }
...
async function askQuestion() {
  if (!aiStore.isModelLoaded || !chatInput.value.trim() || !readerStore.content)
    return;
  ...
  try {
    await aiStore.askAboutBook(question, readerStore.content, {
      useHistory: true,
...
async function exportReport() {
  ...
  try {
    const markdown = await exportAIAnalysis({
      ...
      chapters: readerStore.catalog.map((ch, idx) => ({
        index: idx,
    ...
    downloadMarkdown(
      markdown,
      `${readerStore.currentBook.name || "AI分析"}_分析报告.md`
    );
    success("导出成功");
  } catch (e) {
...
<template>
  <Sheet :open="open" @update:open="emit('update:open', $event)">
    <SheetContent side="right" class="w-[400px] sm:w-[450px] overflow-y-auto">
      <SheetHeader class="mb-6">
        <div class="flex items-center justify-between">
          <SheetTitle class="flex items-center gap-2.5">
            <div
              class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shadow-inner"
            >
              <Sparkles
            ...
            <div class="flex flex-col">
              ...
              <span
                class="text-[10px] text-muted-foreground font-medium uppercase tracking-[0.2em]"
                >Assistant</span
              >
            </div>
          </SheetTitle>
          <div class="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              class="rounded-xl border-white/10 shadow-sm font-bold text-[11px]"
              @click="goToSettings"
            >
      ...
      <!-- 动态装饰背景 -->
      <div
        class="absolute inset-0 pointer-events-none -z-10 bg-gradient-to-br from-primary/5 via-transparent to-blue-500/5"
      />

      <!-- WebGPU 不支持提示 -->
      <div
        v-if="!aiStore.isSupported"
        class="p-5 rounded-2xl bg-destructive/10 border border-destructive/20 text-destructive text-sm backdrop-blur-md"
      >
        ...
        <p class="opacity-80 text-xs">
          您的硬件或浏览器不支持 WebGPU 协议。请尝试更新 Chrome 至最新版本。
        </p>
      </div>

      <!-- 模型未加载 -->
      <div
        v-else-if="!aiStore.isModelLoaded"
        class="py-20 text-center animate-in fade-in zoom-in-95 duration-700"
      >
        <div
          class="w-20 h-20 rounded-[2.5rem] bg-muted/50 flex items-center justify-center mx-auto mb-6 shadow-premium ring-1 ring-white/10"
        >
          <Brain class="h-10 w-10 text-muted-foreground/30" />
        </div>
        <h3 class="text-lg font-black mb-2 tracking-tight">思维模块离线</h3>
        <p
          class="text-xs text-muted-foreground mb-8 max-w-[200px] mx-auto leading-relaxed"
        >
        ...
        <Button
          @click="goToSettings"
          size="lg"
          class="rounded-2xl shadow-premium font-bold tracking-tight"
        >
      ...
      <div v-else class="space-y-6 animate-in fade-in duration-500">
        <!-- 模型状态仪表盘 -->
        <div
          class="relative overflow-hidden p-4 rounded-2xl bg-muted/30 border border-white/10 shadow-premium"
        >
          <div
            v-if="isProcessing"
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2.5">
              <div
                ...
                :class="isProcessing ? 'bg-yellow-500' : 'bg-green-500'"
              />
              <span
                class="text-[11px] font-black uppercase tracking-widest text-foreground/80"
                >{{ aiStore.currentModel?.split("-")[0] }}</span
              >
            </div>
            <span
              v-if="fromCache"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="p-2 rounded-xl bg-background/40 border border-white/5">
              <p
                class="text-[9px] text-muted-foreground uppercase font-black tracking-tighter mb-1"
              >
                推断速度
              </p>
              <div class="flex items-baseline gap-1">
                <span class="text-sm font-mono font-bold">{{
                  aiStore.performance?.tokensPerSecond || "--"
                }}</span>
            ...
            <div class="p-2 rounded-xl bg-background/40 border border-white/5">
              ...
              <div class="flex items-baseline gap-1">
                <span class="text-sm font-mono font-bold">{{
                  aiStore.performance?.generationTime?.toFixed(2) || "--"
                }}</span>
        ...
        <!-- 极简功能标签页 -->
        <div
          class="flex gap-1 p-1.5 rounded-2xl bg-muted/50 border border-white/5 flex-wrap"
        >
          <button
            v-for="tab in ([
            ...
            :class="
              activeTab === tab.id
                ? 'bg-background text-primary shadow-premium'
        ...
        <div class="animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          <div v-if="activeTab === 'summary'" class="space-y-4">
            ...
            <div v-if="summaryResult" class="relative group">
              <div
                class="absolute -inset-0.5 bg-gradient-to-r from-primary/20 to-blue-500/20 rounded-2xl blur opacity-0 group-hover:opacity-100 transition duration-500"
              />
              <div
                class="relative p-5 rounded-2xl bg-background border border-white/10 text-[13px] leading-relaxed shadow-premium"
              >
                {{ summaryResult
                }}<span
                  v-if="isStreaming && activeTab === 'summary'"
                  class="inline-block w-1.5 h-4 bg-primary ml-1 animate-pulse rounded-full"
                />
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'recap'" class="space-y-4">
            ...
            <div
              v-if="recapResult"
              class="p-5 rounded-2xl bg-background border border-white/10 text-[13px] leading-relaxed whitespace-pre-wrap shadow-premium"
            >
              {{ recapResult }}
            </div>
          ...
          <div v-if="activeTab === 'homophone'" class="space-y-4">
            ...
            <div v-if="homophoneResult.length > 0" class="space-y-2.5">
              ...
              >
                <div class="flex items-center gap-3">
                  <span class="text-[13px] font-black text-muted-foreground">{{
                    item.original
                  }}</span>
                  <div class="w-4 h-px bg-white/10" />
                  <span class="text-[13px] text-primary font-black">{{
                    item.guess
                  }}</span>
                </div>
                <div class="flex items-center gap-3">
            ...
            <div
              v-else-if="!isProcessing && hasAnalyzed"
              class="text-center py-12 bg-muted/20 border border-white/5 rounded-2xl"
            >
            ...
            <div v-if="slangResult.length > 0" class="space-y-2.5">
              ...
              >
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <!-- 来源标记 -->
                  <span
                    v-if="(item as any).source === 'local'"
                    class="text-[9px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-600"
                    >📚 内置</span
                  >
                  <span
                    v-else-if="(item as any).source === 'search'"
                    class="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-600"
                    >🔍 搜索</span
                  >
                  <span
                    v-else-if="(item as any).source === 'cache'"
                    class="text-[9px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600"
                    >⚡ 缓存</span
                  >
                </div>
          ...
          <div v-if="activeTab === 'memes'" class="space-y-4">
            ...
            <div v-if="memesResult.length > 0" class="space-y-2.5">
              <div
                ...
                class="p-4 rounded-2xl bg-background border border-white/5 group hover:border-primary/30 transition-all shadow-sm"
              >
                <p class="text-[13px] font-black text-primary mb-1">
                  「{{ item.reference }}」
                </p>
          ...
          <div v-if="activeTab === 'chat'" class="space-y-5">
            ...
            <!-- 当前回复 -->
            <div
              v-if="chatResult && isStreaming"
              class="flex justify-start animate-in fade-in duration-300"
            >
              <div
                class="max-w-[85%] px-4 py-3 rounded-2xl rounded-tl-none bg-muted/80 backdrop-blur-md border border-white/5 text-[13px] leading-relaxed"
              >
                {{ chatResult
                }}<span
                  class="inline-block w-1.5 h-4 bg-primary ml-1 animate-pulse rounded-full"
            ...
            <div class="relative group">
              <div
                class="absolute -inset-1 bg-gradient-to-r from-primary/10 to-blue-500/10 rounded-2xl blur-md opacity-0 group-focus-within:opacity-100 transition"
              />
              <div
                class="relative flex gap-2 p-1 bg-muted/30 backdrop-blur-xl border border-white/10 rounded-[1.25rem]"
              >
                <input
                  v-model="chatInput"
                  type="text"
