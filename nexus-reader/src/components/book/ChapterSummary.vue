...
async function loadSummary(force = false) {
  ...
  try {
    ...
    logger.error("生成章节摘要失败", e as Error, {
...
// 监听 AI 模型加载状态
watch(
  () => aiStore.isModelLoaded,
  (loaded) => {
    if (
      loaded &&
      settingsStore.config.autoSummary &&
      !summary.value &&
      !isLoading.value &&
      isMountedRef.value
    ) {
...
<template>
  ...
  >
    <!-- 标题栏 -->
    <div
      class="flex items-center gap-2 mb-2 text-primary/80 font-medium text-sm"
    >
