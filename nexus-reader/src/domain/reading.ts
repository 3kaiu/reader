...
import { AggregateRoot, ValueObject, DomainEvent, DomainResult, DomainError, BusinessRuleValidator, getDomainLayer } from './index';
...
export class ReadingSettingsChangedEvent implements DomainEvent {
  eventId: string;
  eventType = 'ReadingSettingsChanged';
  timestamp: Date;
  aggregateId: string;
  eventData: Record<string, any>;

  constructor(userId: string, settings: ReadingSettings) {
    this.eventId = crypto.randomUUID();
    this.timestamp = new Date();
 ...
 * 阅读设置值对象
 */
export class ReadingSettings implements ValueObject {
  fontSize: number;
  fontFamily: string;
  lineHeight: number;
  ...
  constructor(settings: Partial<ReadingSettings> = {}) {
  ...
  equals(other: ValueObject): boolean {
    if (!(other instanceof ReadingSettings)) return false;
    return (
...
export class ReadingSession implements AggregateRoot {
  ...
  constructor(session: Partial<ReadingSession>) {
    ...
    this.settings = session.settings ?? new ReadingSettings();
...
export class ReadingPreferencesService {
  ...
  async optimizeSettings(userId: string, device: DeviceInfo): Promise<DomainResult<ReadingSettings>> {
    const startTime = Date.now();

    try {
      // 基于设备信息优化阅读设置
      let settings = new ReadingSettings();
      ...
      return {
        success: true,
        data: settings,
        events: [new ReadingSettingsChangedEvent(userId, settings)],
...
export class ReadingProgressValidRule implements BusinessRuleValidator<ReadingSession> {
  ...
  validate(entity: ReadingSession, context: any): void {
 ...
 * 阅读设置验证规则
 */
export class ReadingSettingsValidRule implements BusinessRuleValidator<ReadingSession> {
  ...
  validate(entity: ReadingSession, context: any): void {
    ...
    if (settings.fontSize < 12 || settings.fontSize > 32) {
      throw new DomainError(
        'Font size must be between 12 and 32',
        'VALIDATION_ERROR',
        'reading'
      );
    }
...
export interface ReadingDomainState {
  ...
  userPreferences: ReadingSettings;
  bookmarks: Bookmark[];
}
...
export class ReadingDomainStateManager {
  ...
  getState(): ReadingDomainState {
    return this.stateManager.getState<ReadingDomainState>('reading') ?? {
      recentBooks: [],
      readingStats: {
        totalSessions: 0,
        totalReadingTime: 0,
      ...
      userPreferences: new ReadingSettings(),
      bookmarks: [],
    };
  }
   ...
   * 更新用户偏好设置
   */
  updateUserPreferences(preferences: ReadingSettings): void {
    const state = this.getState();
