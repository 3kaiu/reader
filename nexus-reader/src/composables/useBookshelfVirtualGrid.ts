...
export function useBookshelfVirtualGrid<T>(items: Ref<readonly T[]>) {
  ...
  const columnsPerRow = computed(() =>
    getBookshelfColumnsPerRow(windowWidth.value || 1280)
  )
  const rows = computed(() =>
    Math.ceil(items.value.length / Math.max(columnsPerRow.value, 1))
  )
  const shouldUseVirtualScroll = computed(
    () => items.value.length > VIRTUAL_SCROLL_THRESHOLD
  )
