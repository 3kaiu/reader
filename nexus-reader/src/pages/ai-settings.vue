...
onMounted(async () => {
  ...
  // 自动恢复上次加载的模型（如果已缓存）
  if (aiStore.isSupported && !aiStore.isModelLoaded && !aiStore.isLoading) {
    const lastModel = localStorage.getItem('ai-last-model');
    if (lastModel) {
      // 检查模型是否已缓存
            ...
            >
              ...
              <div class="space-y-2">
                ...
                <input
                  ...
                  @input="(e: Event) => settingsStore.updateConfig('aiParams', { ...settingsStore.config.aiParams, topP: parseFloat((e.target as HTMLInputElement).value) })"
                  class="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer accent-primary"
                />
              </div>
              ...
              <div class="space-y-2">
                ...
                <input
                  ...
                  @input="(e: Event) => settingsStore.updateConfig('aiParams', { ...settingsStore.config.aiParams, presencePenalty: parseFloat((e.target as HTMLInputElement).value) })"
                  class="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer accent-primary"
                />
              </div>
              ...
              <div class="space-y-2">
                ...
                <input
                  ...
                  @input="(e: Event) => settingsStore.updateConfig('aiParams', { ...settingsStore.config.aiParams, frequencyPenalty: parseFloat((e.target as HTMLInputElement).value) })"
                  class="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer accent-primary"
                />
              </div>
            </div>
